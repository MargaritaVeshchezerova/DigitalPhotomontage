...
void test2(char* database_name, char* result_name)
{
	...
	*/
	...
	loadDatabase(database_name, *images);
	int n = images->size();
	...
	Mat Label = Mat::zeros(rows, cols, CV_32SC1); //on veut stocker les label des differents trucs; comme ca l'image est le pixel de l'image caracterise par son label
	...
	imwrite(string(result_name)+".jpg", result);
